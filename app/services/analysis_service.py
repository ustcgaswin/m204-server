...
async def perform_source_file_analysis(db: Session, input_source_id: int) -> UnifiedAnalysisReportSchema:
    ...
    try:
        ...
        if file_type == "parmlib":
            ...
            analysis_details, m204_files_from_jcl = await jcl_analysis_service.process_jcl_analysis(db, input_source, file_content, current_rag_service_instance)
        ...
        else: # No errors, no details (e.g. unhandled file type that doesn't produce an error)
            analysis_status_val = "completed"
