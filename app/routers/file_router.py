from fastapi import APIRouter, Depends, UploadFile, File, HTTPException, status # Removed Form
...
async def upload_source_files(
    ...
    log.info(f"Received {len(files)} files for upload request for project_id: {project_id}.")
    ...
    for single_file in files:
        if not single_file.filename:
            log.warning(f"Upload attempt for project {project_id} included a file with no filename.")
            # Create a generic error message as we don't have a user-defined segment to reference
            errors_occurred.append("A file was provided without a filename and was skipped.")
            continue
        
        # Use the file's own name as the path segment
        file_path_segment_from_filename = single_file.filename
            
        try:
            log.info(f"Processing file: '{single_file.filename}' for project_id: {project_id} (using filename as path segment: '{file_path_segment_from_filename}')")
            ...
            log.info(f"Successfully processed upload for project_id: {project_id}, file: '{single_file.filename}', input_source_id: {input_source_data.input_source_id}, stored_path: '{input_source_data.file_path_or_identifier}'")
        except HTTPException as he:
            log.error(f"HTTPException during upload of file '{single_file.filename}' for project {project_id}: {he.detail}")
            errors_occurred.append(f"File '{single_file.filename}': {he.detail}")
        except Exception as e:
            log.error(f"Unexpected error during upload of file '{single_file.filename}' for project {project_id}: {str(e)}", exc_info=True)
            errors_occurred.append(f"File '{single_file.filename}': An unexpected error occurred.")
    ...
    return ListResponse[InputSourceResponseSchema]( # Explicitly type ListResponse here
        message=message,
...
async def list_source_files_for_project(
    ...
    if not source_files_data and total_items == 0 :
        ...
        # Provide an explicitly typed empty list for data
