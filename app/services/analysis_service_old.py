    ...
    for i, llm_result_or_exc in enumerate(llm_results_list):
        ...
        if existing_proc:
            ...
            if existing_proc.start_line_in_source != db_data_for_procedure["start_line_num"]:
                existing_proc.start_line_in_source = db_data_for_procedure["start_line_num"]
                update_needed = True
            if existing_proc.end_line_in_source != db_data_for_procedure["end_line_num"]:
                existing_proc.end_line_in_source = db_data_for_procedure["end_line_num"]
                update_needed = True
            if existing_proc.m204_proc_type != db_data_for_procedure["actual_proc_type"]:
                existing_proc.m204_proc_type = db_data_for_procedure["actual_proc_type"]
                update_needed = True
            if existing_proc.m204_parameters_string != db_data_for_procedure["params_str"]:
                ...
                existing_proc.parsed_parameters_json = db_data_for_procedure["parsed_params_json"]
                update_needed = True
            ...
            if hasattr(existing_proc, 'procedure_content') and existing_proc.procedure_content != db_data_for_procedure["current_procedure_content"]:
                existing_proc.procedure_content = db_data_for_procedure["current_procedure_content"]
                update_needed = True
            if db_data_for_procedure["generated_summary"] is not None and existing_proc.summary != db_data_for_procedure["generated_summary"]:
                existing_proc.summary = db_data_for_procedure["generated_summary"]
                update_needed = True
            if db_data_for_procedure["suggested_cobol_name"] is not None and existing_proc.target_cobol_program_name != db_data_for_procedure["suggested_cobol_name"]:
                existing_proc.target_cobol_program_name = db_data_for_procedure["suggested_cobol_name"]
                update_needed = True
    ...
    for proc_data_no_llm in procedures_not_undergoing_llm_data:
        ...
        if existing_proc:
            ...
            if existing_proc.start_line_in_source != proc_data_no_llm["start_line_num"]:
                existing_proc.start_line_in_source = proc_data_no_llm["start_line_num"]
                update_needed = True
            if existing_proc.end_line_in_source != proc_data_no_llm["end_line_num"]:
                existing_proc.end_line_in_source = proc_data_no_llm["end_line_num"]
                update_needed = True
            if existing_proc.m204_proc_type != proc_data_no_llm["actual_proc_type"]:
                existing_proc.m204_proc_type = proc_data_no_llm["actual_proc_type"]
                update_needed = True
            if existing_proc.m204_parameters_string != proc_data_no_llm["params_str"]:
                ...
                existing_proc.parsed_parameters_json = proc_data_no_llm["parsed_params_json"]
                update_needed = True
            ...
            if proc_data_no_llm["suggested_cobol_name"] is not None and existing_proc.target_cobol_program_name != proc_data_no_llm["suggested_cobol_name"]:
                existing_proc.target_cobol_program_name = proc_data_no_llm["suggested_cobol_name"]
                update_needed = True
...
async def _extract_and_store_m204_variables(
    ...
    for i, line_content in enumerate(lines):
        ...
        for match in define_var_pattern.finditer(line_content):
            ...
            scope = "LOCAL"
            if scope_keyword_before:
                scope = scope_keyword_before.upper()
            elif scope_keyword_after:
                scope = scope_keyword_after.upper()
        ...
        for item in all_extracted_items:
            if item in db.dirty or item in db.new:
                try: 
                    ...
                    log.warning(f"Could not refresh item {item} after commit: {e_refresh}")
        ...
        for dd in extracted_dd_statements: 
            if dd in db.dirty or dd in db.new: 
                try: 
                    ...
                    log.warning(f"Could not refresh DD statement {dd.dd_statement_id} after commit: {e_refresh_dd}")
        ...
        for m204file in m204_files_from_jcl:
            if m204file in db.dirty or m204file in db.new:
                try:
                    db.flush()
                    db.refresh(m204file)
                except Exception as e_refresh_m204:
                    log.warning(f"Could not refresh M204File {m204file.m204_file_id} (from JCL) after commit: {e_refresh_m204}")
        ...
        for field in defined_fields_extracted: 
            if field in db.dirty or field in db.new: 
                try: 
                    ...
                    log.warning(f"Could not refresh M204Field {field.m204_field_id} after commit: {e_refresh}")
        ...
        for m204file in m204_files_from_parmlib: # Refresh M204Files from PARMLIB context
             if m204file in db.dirty or m204file in db.new:
                try:
                    db.flush()
                    db.refresh(m204file)
                except Exception as e_refresh_m204_parm:
                    log.warning(f"Could not refresh M204File {m204file.m204_file_id} (from PARMLIB) after commit: {e_refresh_m204_parm}")
